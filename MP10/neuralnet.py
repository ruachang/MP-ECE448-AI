...
class NeuralNet(nn.Module):
    def __init__(self, lrate, loss_fn, in_size, out_size):
        ...
        self.conv1 = nn.Sequential(
            # 64 * 27 * 27
            nn.Conv2d(in_channels=3, out_channels=32, kernel_size=5, stride=1, padding=1, bias=False),
            # nn.BatchNorm2d(32),
            nn.ReLU(),
        ...
        #    nn.BatchNorm2d(16),
        #    nn.ReLU(),
        #   nn.Conv2d(in_channels=16, out_channels=16, kernel_size=3, stride=1, padding=0, bias=False),
        #    nn.BatchNorm2d(16),
        #    nn.ReLU(),
        #    nn.MaxPool2d(kernel_size=2),
        # )
        
        self.linear1 = nn.Linear(64 * 6 * 6, 4)
        # self.linear2 = nn.Linear(36, 4)
        self.relu = nn.ReLU()
...
def combine_data(image, label):
    ...
    return data
def loss_func(criterion, y_hat, y, lambda_weight = 0.0001):
    loss = criterion(y_hat, y) #+ lambda_weight * 
    return loss

# def confusion_mat()

def fit(train_set,train_labels,dev_set,epochs,batch_size=32):
    """ 
    ...
    # train_set = data_std(train_set)
    # trainLoader2 = torch.utils.data.DataLoader(train_set, batch_size=1, shuffle=False)
    ...
    # TODO load data
    losses = []
    for epoch in range(int(30)):
    # for epoch in range(21):
        loss = train(model, lrate, trainLoader)
        losses.append(loss)
    model.optimizer.param_groups[0]['lr'] = lrate / 10
    for epoch in range(int(20)):
        loss = train(model, lrate, trainLoader)
        losses.append(loss)
    # train_yhats = test(model, trainLoader2)
    # acc = 0
    # for i in range(len(train_yhats)):
    #     if train_yhats[i] == train_labels[i]:
    #         acc += 1 / len(train_yhats)
