    ...
    def execute(self):
        if self.args.early_check:
            ...
            for i in range(len(envs)):
                if i == 76:
                    test_results[i] = self.agent.generate_state(envs[i])
            differences = np.where(gold_results != test_results)

            # Print out the differing elements
            for index in zip(*differences):
                print(f"Difference at index {index}: gold_results has {gold_results[index]}, test_results has {test_results[index]}")
            ...
            for game in range(1, len(actions_list) + 1):
                ...
                while not dead and count < len(actions_list[game - 1]):
                    ...
                    if s_prime == (2, 1, 0, 0, 0, 0, 0, 0):
                        print("stop")
                    if self.agent.s == (0, 1, 0, 0, 0, 0, 1, 0):
                        print("stop")
                ...
                # print(gold_N[1, 1, 0, 0, 0, 0, 0, 0])
                # print(self.agent.N[1, 1, 0, 0, 0, 0, 0, 0])
                differences_Q = np.where(gold_Q != self.agent.Q)
                differences_N = np.where(gold_N != self.agent.N)

                # Print out the differing elements
                for index in zip(*differences_Q):
                    print(f"Difference at index {index}: gold_Q has {gold_Q[index]}, self.agent.Q has {self.agent.Q[index]}")
                for index in zip(*differences_N):
                    print(f"Difference at index {index}: gold_N has {gold_N[index]}, self.agent.N has {self.agent.N[index]}")
