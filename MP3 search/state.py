...
class AbstractState(ABC):
    def __init__(self, state, goal, dist_from_start=0, use_heuristic=True):
        ...
        # dist_from_start is cnextlassically called "g" when describing A*, i.e., f(state) = g(start, state) + h(state, goal)
...
# EightPuzzle ------------------------------------------------------------------------------------------------
# find value in a puzzle
def find_value(puzzle, target):
    for i in range(len(puzzle)):
        for j in range(len(puzzle[i])):
            if puzzle[i][j] == target:
                return (i, j)

def move_puzzle(puzzle, zero_x, zero_y, array):
    new_puzzle = copy.deepcopy(puzzle)     
    if array == 0:
        if zero_x == 2:
            return None, None
        else:
            new_puzzle[zero_x][zero_y] = new_puzzle[zero_x + 1][zero_y]
            new_puzzle[zero_x + 1][zero_y] = 0
            zero_loc = (zero_x + 1, zero_y)
    if array == 1:
        if zero_y == 0:
            return None, None
        else:
            new_puzzle[zero_x][zero_y] = new_puzzle[zero_x][zero_y - 1]
            new_puzzle[zero_x][zero_y - 1] = 0
            zero_loc = (zero_x, zero_y - 1)
    if array == 2:
        if zero_x == 0:
            return None, None
        else:
...
class EightPuzzleState(AbstractState):
    ...
    def get_neighbors(self):
        ...
        zero_x, zero_y = self.zero_loc
        for i in range(4):
            # * below(zero)
            new_puzzle, new_zero_loc = move_puzzle(self.state, zero_x, zero_y, i)
            if new_puzzle != None:
                new_state = EightPuzzleState(state = new_puzzle,
                                     goal = self.goal,
    ...
    def compute_heuristic(self):
        ...
        for i in range(1, 9):
            now_pos = find_value(self.state, i)
            target_pos = find_value(self.goal, i)
            total += manhattan(now_pos, target_pos)
        return total
